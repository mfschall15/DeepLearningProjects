...
augment_data = False
# train_dataset = TASDataset('drive/MyDrive/tas500v1.1', augment_data=augment_data) 
# val_dataset = TASDataset('drive/MyDrive/tas500v1.1', eval=True, mode='val')
# test_dataset = TASDataset('drive/MyDrive/tas500v1.1', eval=True, mode='test')
...
# loss_weights = get_loss_weights(weighting_method='INS')
...
def train():
    ...
    print("learning-rate: {}    batch-size: {}  total-epochs: {}    early-stop: {}  data-augmentation: {}".format(lr, batchsize, epochs, epoch_stop, augment_data))
    epoch_no_imp = 0
...
def test():
    ...
    with torch.no_grad(): # we don't need to calculate the gradient in the validation/testing
        ...
        for iter, (input, label) in enumerate(test_loader):
            ...
            if is_first:
                visualize_prediction(input[0], pred[0])
                is_first = False
